...
const Explore = () => {
   ...
   const [downloadingstate, seDownloadingstate] = useState(false);
   ...
   axios.post('http://localhost/api/user.php',null,config).then((result)=>{
    ...
    <img className='rounded-xl cursor-pointer  group-hover:brightness-75 transition-all duration-200 '  src={'http://localhost/api/images/'+image.name} /> 
     ...
     <div className="absolute rounded-full bottom-3 right-0 -translate-x-1/2">
      <button  disabled={downloadingstate} className='shadow-lg z-20 rounded-xl h-10 w-10 bg-gray-200 flex justify-center items-center  transition-all duration-200 scale-0 origin-right  group-hover:scale-100'  onClick={download(image)}>
