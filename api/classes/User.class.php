...
public function Uplod($FILES,$data)
{
    ...
    $address = !empty($data['latitude']) ? htmlspecialchars($data['address'], ENT_QUOTES, 'UTF-8') : NULL;
...
public function changeSubscription($userid,$value){
  $query = 'UPDATE subscription SET plan = :plan WHERE userid = :userid';
  $statement = $this->conn->prepare($query); 
  $statement->bindValue(":userid",$userid); 
  $statement->bindValue(":plan",$value); 
  $statement->execute();
}

 public function FetchSubscription($userid){
    $query = 'SELECT * FROM subscription WHERE userid = :userid';
    $statement = $this->conn->prepare($query); 
    $statement->bindValue(":userid",$userid); 
    $result = $statement->execute();
    foreach ($statement->fetchAll() as $value) {
         return json_encode(array("plan"=>ucfirst($value['plan']),"subscribed"=>$value['subscribed'],"expiry" => $value['expired']));
    }
